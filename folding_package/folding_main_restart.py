...
from folding_package.traj_gen_com import traj_gen_com
from folding_package.res_ids import res_ids
from folding_package.plumed_script import plumed_script_com
...
for iteration in range(0, num_iter):
    ...
    if os.path.isfile('output/ml/'+'data.csv'):
        print('Previosly generated order parameters are imported')
        OPs = pd.read_csv('output/ml/'+'data.csv', sep=' ')
        OPs = OPs.drop(OPs.columns[0], axis=1)
    else:
        print('New order parameters are generated')
        OPs =traj_gen_com(number_of_bases, max_id, min_id, 1, rna)
        OPs.to_csv('output/ml/'+'data.csv', sep=' ')
    #OPs =traj_gen_com(number_of_bases, max_id, min_id, 10, rna)
        
        
    #initialize AMINO package
    if not os.path.isfile('input/data_0'):
        print('data_0 file does not exist.')    
        if iteration==0:
        ...
        OPs_reduced=pd.read_csv('input/data_0', sep=' ')
        OPs_reduced=OPs_reduced.drop(OPs_reduced.columns[0], axis=1)
        print('data_0 file exists.')
    ...
    if iteration == 0:
        ...
        bias_factor=10
        label='RC_1'
        sigma= abs((OPs_reduced.std()[0:RCs.shape[1]-1].values*RCs.iloc[len(time_delay)-1,1:RCs.shape[1]].values).sum())*0.1
        print("Metadynamics sigma value:" + str(sigma))
        height=0
        pace=100
        #plumed_script(OPs_reduced, RCs, iteration, T, bias_factor, label, sigma, height, pace, 'output/meta/')
        plumed_script_com(OPs_reduced, number_of_bases, min_id, max_id, time_delay, rna, RCs, iteration, T, bias_factor, label, sigma, height, pace, 'output/meta/')
