...
def plumed_script_double_same(OPs_1_r, OPs_2_r, number_of_bases, ref_atom_id, time_delay, universe_name, RCs_1, RCs_2, iteration, T, bias_factor, sigma_1, sigma_2, height, pace, output_path):
    ...
    try:
        ...
        try:
            ...
            file.write("WHOLEMOLECULES ENTITY0="+"1-588"+'\n')
            ...
            for i in range(0, len(OPs_1_r.columns)):    
                ...
                if rc_name not in rc_name_l:
                   ...
                   ### str1=OPs_1_r.columns[i][OPs_1_r.columns[i].find('_')+len('_'):OPs_1_r.columns[i].rfind('')]
                    j=0
                    dec=0
                    resid_nums=[]
                    #print(len(list(str3))-2)
                    #for k in range(0, len(str2)):
                    if j<len(list(str1))-1:
                        resid_nums.append(list(str1)[j]+list(str1)[j+1])
                        print(resid_nums)
                        print('prnting resid nums')
                    if j>=len(list(str1))-1:
                        print(list(str1)[j])
            ...
            file.write('1-588 '+'LABEL=rg '+'NOPBC'+'\n')
...
def plumed_script_double_same_mixedops(OPs_1_r, OPs_2_r, number_of_bases, ref_atom_id, stack_atom_id, time_delay, universe_name, RCs_1, RCs_2, iteration, T, bias_factor, sigma_1, sigma_2, height, pace, output_path):
    """Automatically generate plumed file with COM ATOMS for both parts of reaction coordinates"""
    
    try:
        with open(output_path + 'plumed_s_' + str(iteration) + '.dat', "w+") as file:
            file.write("RESTART\n")
            file.write("WHOLEMOLECULES ENTITY0=1-588\n")
            
            # Store reaction coordinates for COM ATOMS without duplicates
            rc_name_set = set()

            def process_coordinate(header):
                """Helper function to extract numbers from headers and write COM ATOMS"""
                match = re.match(r'([ds])(\d+)_(\d+)', header)
                if match:
                    first_part = match.group(2)
                    second_part = match.group(3)

                    # Generate C or S labels (e.g., C4, C7 or S4, S6)
                    for part in [first_part, second_part]:
                        label = match.group(1).upper() + part  # C or S followed by residue number
                        if label not in rc_name_set:
                            rc_name_set.add(label)
                            file.write(f'{label}: COM ATOMS=')
                            
                            # Get residue IDs and write them to the file
                            resid_nums = [part]
                            if match.group(1) == 'd':
                                atom_ids = res_ids_n(" ".join(resid_nums), ref_atom_id, universe_name)
                            else:
                                atom_ids = res_ids_n(" ".join(resid_nums), stack_atom_id, universe_name)
                            file.write(','.join([str(atom_id + 1) for atom_id in atom_ids]))
                            file.write('\n')

            # Process OPs_1_r for reaction coordinates and write COM ATOMS
            for header in OPs_1_r.columns:
                process_coordinate(header)

            # Process OPs_2_r for reaction coordinates and write COM ATOMS
            for header in OPs_2_r.columns:
                process_coordinate(header)

            # Write DISTANCE commands (corrected to use COM labels)
            for header in OPs_1_r.columns:
                if header.startswith('d') or header.startswith('s'):
                    file.write(f'{header}: DISTANCE ATOMS=')
                    parts = header.split('_')
                    # Use the COM labels (e.g., S5,S8 instead of 5,8)
                    first_label = parts[0][0].upper() + parts[0][1:]  # e.g., 's5' -> 'S5'
                    second_label = parts[0][0].upper() + parts[1]     # e.g., '8' -> 'S8' (assuming same prefix)
                    file.write(f"{first_label},{second_label} NOPBC\n")

            # Combine the OPs for RC_1
            file.write("\nCOMBINE LABEL=RC_1 ARG=")
            file.write(",".join([header for header in OPs_1_r.columns if header.startswith('d') or header.startswith('s')]))
            file.write(' COEFFICIENTS=')
            file.write(','.join([str(RCs_1.iloc[len(time_delay) - 1, i]) for i in range(1, RCs_1.shape[1])]))
            file.write(' PERIODIC=NO\n\n\n\n')

            # Combine the OPs for RC_2
            file.write("\nCOMBINE LABEL=RC_2 ARG=")
            file.write(",".join([header for header in OPs_2_r.columns if header.startswith('d') or header.startswith('s')]))
            file.write(' COEFFICIENTS=')
            file.write(','.join([str(RCs_2.iloc[len(time_delay) - 1, i]) for i in range(1, RCs_2.shape[1])]))
            file.write(' PERIODIC=NO\n\n\n\n')

            # Define RADIUS OF GYRATION
            file.write("\nGYRATION TYPE=RADIUS ATOMS=1-588 LABEL=rg NOPBC\n")

            # Metadynamics settings
            file.write(f"METAD LABEL=metadynamics BIASFACTOR={bias_factor} TEMP={T} ARG=RC_1,RC_2 SIGMA={sigma_1},{sigma_2} HEIGHT={height} PACE={pace} GRID_MIN=-15,-15 GRID_MAX=15,15 GRID_BIN=256,256 CALC_RCT RCT_USTRIDE=1\n\n\n\n")

            # Print setup
            file.write("PRINT ARG=RC_1,RC_2,")
            file.write(",".join([header for header in OPs_1_r.columns if header.startswith('d') or header.startswith('s')]) + ",")
            file.write(",".join([header for header in OPs_2_r.columns if header.startswith('d') or header.startswith('s')]))
            file.write(f",rg,metadynamics.bias,metadynamics.rct STRIDE={pace} FILE=BIASED_COLVAR\n")
