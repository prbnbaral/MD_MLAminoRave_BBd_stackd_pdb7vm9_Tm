...
def arpm(iteration):
    ...
    topparfile='input/md/'+'toppar_drude_23.str'
    
    if iteration == 0:
        dcdfile='input/md/'+'step5_1.dcd'
    ...
    tot_num_aas = traj_sys.select_atoms('resname ALA or resname ARG or resname ASN or resname ASP or resname CYS or resname GLN or \
                                         resname GLU or resname GLY or resname HSD or resname HSE or resname HSP or resname HIS or \
                                         resname ILE or resname LEU or resname LYS or resname MET or resname PHE or resname PRO or \
                                         resname SER or resname THR or resname TRP or resname TYR or resname VAL')
    #print(len(set(num_aas.resids)))

    tot_aas = int(len(set(tot_num_aas.resids)))
    #print(tot_aas)

    ref_id = str(traj_sys.select_atoms(f'protein and name N DN CA DCA C DC O DO LPOA LPOB').ids)
    ...
    if not os.path.isfile('input/data_1_0'):
        ...
        if iteration==0:
             ...
             final_ops = find_ops(all_ops, 3, 50)
...
#       file.close()
    ...
    if iteration > 0:
        ...
        if os.path.isfile('output/meta/'+'plumed_s_'+str(iteration)+'.dat'):
            ...
            plumed_script_double_same(OPs_1_r, OPs_2_r, tot_aas, ref_id, time_delay, traj_sys, RCs_1, RCs_2, iteration, 339.00, bias_factor, sigma_1, sigma_2, height, pace, 'output/meta/')
            file = open('output/meta/'+'plumed_s_'+str(iteration)+'.dat',"r+")
            script = file.read()
            file.close()
